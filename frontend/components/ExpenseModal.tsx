      ...
      if (result.success) {
        ...
        if (result.category) {
          ...
          if (match) {
            ...
            setAnalysisInfo(`Categoría sugerida: "${result.category}" (no encontrada en tu lista)`)
          }
        }

        if (filled.length > 0) {
          toast.success(`Extraído de la factura: ${filled.join(', ')}`)
        } else {
          toast('No se encontraron datos en la factura', { icon: '⚠️' })
        }
      } else {
          ...
          {!isEditing && (
            <div className="mb-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-slate-400">(opcional · extrae datos automáticamente)</span>
              </div>
              ...
              {pendingFile ? (
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 space-y-2">
                  ...
                  {analyzing && (
                    <p className="text-xs text-indigo-600 font-medium">Analizando factura con IA...</p>
                  )}
